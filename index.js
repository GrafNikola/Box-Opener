module.exports = function BoxOpener(mod) {
	...
	mod.command.add(["box", "开盒"], (arg, number) => {
		if (!arg) {
			if (!mod.settings.enabled && !scanning) {
				scanning = true;
				load();
				sendMessage("Please open a box, the script will recycle it");
			} else {
				stop();
			}
		} else {
			switch (arg) {
				...
				case "延迟":
					...
					if (isNaN(number) || number < 1) {
						...
						sendMessage("Requires [Number] type parameter greater than 5, Restore default 0.2 seconds / time");
					} else {
						mod.settings.useDelay = true;
						mod.settings.delay = parseInt(number) * 1000;
						sendMessage("Box opening delay set " + (mod.settings.delay / 1000) + " Sec/times");
					}
					break;
				case "trash":
				case "垃圾":
	...
	function load() {
		hook('C_USE_ITEM', 3, (event) => {
			if (scanning) {
				...
				boxId = event.id;
				sendMessage(
					"\n\t - Select target: "  + boxId + " - " + mod.game.inventory.find(boxId).data.name +
					"\n\t - Open box interval: " + (mod.settings.useDelay ? (mod.settings.delay / 1000) + " Seco/times" : "No delay")
				);
		...
		hook('S_SYSTEM_MESSAGE', 1, (event) => {
			...
			if (msg.id === 'SMT_ITEM_MIX_NEED_METERIAL' || msg.id === 'SMT_CANT_CONVERT_NOW') {
				sendMessage("Can no longer open the box Script stop");
				stop();
			}
			return false;
		});
	...
	function stop() {
		...
		if (scanning) {
			scanning = false;
			sendMessage("Auto-open box script shut down");
		} else {
			clearTimeout(timer);
			...
			sendMessage("boxes open: " + statOpened +
				"\n\t - Total time: " + h + ":" + m + ":" + s +
